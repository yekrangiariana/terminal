...
function renderHelpDemos() {
  ...
  function patternVal(id, mx, my, fw, fh, xC, yC, fm, gV, t) {
    ...
    switch (id) {
      case "centerSpiral":
        dx = mx - hw;
        dy = my - hh;
        theta = atan2(dy, dx) + xC;
        return sin(
          (theta +
            (t + sin(mx * xC + my * yC)) * fm +
            sin(theta * yC) +
            gV * sin(mx * xC + t)) *
            (1 + sin(theta * fm)),
        );
      case "circular":
        dx = mx - hw;
        dy = my - hh;
        return sin(dx * dx * xC + dy * dy * yC + t * fm);
      case "mosaic":
        return sin(mx * xC + t) * sin(my * yC + t);
      case "cross":
        dx = mx - hw;
        dy = my - hh;
        return sin(dx * dx * xC + t * fm) + cos(dy * dy * yC + t * fm);
      case "checkerboard":
      ...
      case "diamond":
        ...
        dy = my - hh;
        return (
          sin((abs(dx) + abs(dy)) * xC + t * fm) *
          cos(yC * atan2(dy, dx) + t * fm * 0.5)
        );
      case "tunnel":
        dx = mx - hw;
        dy = my - hh;
        r = sqrt(dx * dx + dy * dy) + 0.01;
        theta = atan2(dy, dx);
        return (
          sin(xC / r + t * fm) *
          sin(theta * yC + t * fm * 0.7 + gV * sin(r * 0.1))
        );
      case "plasma":
        dx = mx - hw;
        dy = my - hh;
        return (
          (sin(mx * xC * 0.01 + t) +
            sin(my * yC * 0.01 + t * 1.5) +
            sin((mx + my) * xC * 0.005 + t * 0.7) +
            sin(sqrt(dx * dx + dy * dy) * yC * 0.01 + t * 1.2 + gV)) *
          0.25
        );
      case "interference":
        var s1x = fw * 0.3,
        ...
        return (
          (sin(
            sqrt((mx - s1x) * (mx - s1x) + (my - s1y) * (my - s1y)) * xC +
              t * fm +
              gV,
          ) +
            sin(
              sqrt((mx - s2x) * (mx - s2x) + (my - s2y) * (my - s2y)) * yC +
                t * fm,
            )) *
          0.5
        );
      ...
      case "lissajous":
        return (
          sin(mx * xC * 0.1 + sin(t * 3) * gV) *
          sin(my * yC * 0.1 + cos(t * 2) * gV)
        );
      default:
        return sin(mx * 0.1) * sin(my * 0.1);
    }
  }
  ...
  function renderPattern(ctx, w, h, id, xC, yC, fm, gV, opts) {
    ...
    for (var py = 0; py < h; py++) {
      for (var px = 0; px < w; px++) {
        ...
        if (turb) {
          dx +=
            (sin(dx * 0.031 + dy * 0.071) * 0.5 +
              sin(dx * 0.113 - dy * 0.037) * 0.3) *
            turb *
            10;
          dy +=
            (cos(dx * 0.071 + dy * 0.031) * 0.5 +
              cos(dx * 0.037 - dy * 0.113) * 0.3) *
            turb *
            10;
        }
        var mx = dx + fw * 0.5,
        ...
        if (opts.blendPattern) {
          ...
          var bm = opts.blendMode || "add",
            ba = opts.blendAmount || 0.5;
          var bv;
          if (bm === "add") bv = val + vb;
  ...
  document
    .querySelectorAll("#help-content canvas[data-demo]")
    .forEach(function (el) {
      ...
      if (demo === "polar") {
        ...
        ctx.lineWidth = 1.5;
        ctx.beginPath();
        ctx.arc(cx, cy, 22, -ang, 0);
        ctx.stroke();
        ctx.fillStyle = "#333";
        ctx.beginPath();
        ctx.arc(ex, ey, 3, 0, PI * 2);
        ctx.fill();
        ctx.font = "bold 11px sans-serif";
        ctx.fillStyle = "#cc3333";
        ctx.fillText(
          "r",
          cx + cos(ang) * dist * 0.4 - 2,
          cy - sin(ang) * dist * 0.4 - 4,
        );
        ctx.fillStyle = "#2255bb";
        ctx.fillText("\u03b8", cx + 28, cy - 3);
        ctx.fillStyle = "#999";
        ctx.font = "9px sans-serif";
        ctx.fillText("centre", cx - 16, cy + 14);
        return;
      }
      ...
      if (patternDefs[demo]) {
        ...
        renderPattern(ctx, w, h, p.id, p.xC, p.yC, p.fm || 0, p.gV || 0, {
          ...
          scale: p.scale || 1,
          symmetry: p.symmetry || "none",
          turbulence: p.turbulence || 0,
          blendPattern: p.blendPattern,
          blendXC: p.blendXC,
          blendYC: p.blendYC,
          blendFm: p.blendFm,
          blendGV: p.blendGV,

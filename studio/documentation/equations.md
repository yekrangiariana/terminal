...
Mostly **geometry** — shapes, distances, and angles on a flat surface. There's a bit of trigonometry (the maths of circles and waves) and some handy number-shaping tools. No calculus, no physics. That's the whole toolkit.

The core idea: every character on screen sits at a position on a 2D grid. Your formula takes that position and returns a number. That number decides which character gets drawn there. Different numbers at different positions = a pattern.
...
Every point on the grid also has a **distance** from the centre and an **angle** from the centre. These are called polar coordinates — and they're why circular patterns are so easy to make here.
...
`sin` takes any number and returns a value that swings smoothly between −1 and +1, repeating forever. That's a **wave**. It comes from the geometry of circles — if you trace a point going around a circle and plot its height over time, you get this shape:

<canvas data-demo="sin" width="380" height="70" style="display:block;margin:6px 0"></canvas>

`cos` is the exact same shape, just shifted sideways. For pattern-making they're basically interchangeable — pick whichever looks better.
...
See what's happening? `sin(x)` repeats along x, so you get vertical stripes. `sin(y)` repeats along y, horizontal stripes. `sin(r)` repeats outward from the centre — circles. That's most of the maths you need — you'd be surprised how far just sin gets you.
...
- `asin(x)`, `acos(x)`, `atan(x)` — the reverse of sin/cos/tan: give it a wave value, get an angle back. Useful for warping and bending.
- `atan2(y, x)` — finds the angle of any point from the origin. Basically what `theta` is.

### Shaping Numbers

So you've got a wave or a gradient. Now you want to reshape it — sharpen it, flatten it, posterise it. These are the main sculpting tools:
...
- `sqrt(x)` — square root. Big values grow slower, which compresses the bright end. Makes gradients feel wider near the centre.
...
- **`fract(x)`** — keeps only the decimal part (always 0 to 1, then snaps back and repeats). This is the key to tiling: `fract(x * 3)` chops the space into 3 copies of a 0→1 ramp.
...
- **`smoothstep(lo, hi, x)`** — the smooth version of step. Fades gradually from 0 to 1 between lo and hi. No sudden jump, just a gentle S-curve. One of the most useful functions for clean transitions.
...
- **`E`** (≈ 2.72) — base of natural logarithms. Used with `exp()` and `log()`.
- Operators: `+` `-` `*` `/` `%` `()`
